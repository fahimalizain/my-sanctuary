...
> Amendment (2026-08-25): a dedicated **pause** verb joins the occurrence
> matrix: `POST /api/occurrences/:id/pause`. `in_progress` → `pending` after
> the open chip's end is PATCHed closed (same invert guard as complete/skip;
> Google 404 still flips), then the stored chip ids are **cleared** so a later
> start mints a **new** one-shot event — one _living_ chip; the closed event
> stays on Google as an orphaned log. `pending` → 200 no-op (ids never
> cleared). `done`/`skipped` → 400 (`cannot pause a {status} occurrence` —
> reopen is the undo). Reopen of `in_progress` stays 400: pause is the
> Google-writing abort; reopen stays session-only. No fifth status — `pending`
> on the agenda is the plan (task `/pause` lands `PLANNED`). Google token
> refresh matches complete/skip: only when `in_progress` **with** stored ids.
> Home renders Pause on `in_progress` (no today gate — a civil-day rollover
> can still be running). § Occurrence verb matrix / Surfaces (Home) / API
> are amended.
>
...
> after reopen mints a **new** one-shot Google event — one _living_ chip; the
...
> (`cannot reopen an in_progress occurrence` — no abort via reopen: a running
> occurrence exits via complete/skip/pause). `done`/`skipped` remain terminal for `start`
...
| status ↓ / verb → | `start`                        | `complete`                            | `skip`                                   | `reopen`                                                 | `pause`                                                      |
| ----------------- | ------------------------------ | ------------------------------------- | ---------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------ |
| `pending`         | starts — today-only (else 400) | → `done`; no Google                   | → `skipped`; no Google                   | 200 no-op (ids never cleared)                            | 200 no-op (ids never cleared)                                |
| `in_progress`     | 200 no-op                      | → `done`; PATCH open event end closed | → `skipped`; PATCH open event end closed | 400 — no abort via reopen                                | → `pending`; PATCH open event end closed; **clear** chip ids |
| `done`            | 400                            | 200 no-op                             | → `skipped`; no Google                   | → `pending`; **clear** `calendar_id` + `google_event_id` | 400 — reopen is the undo                                     |
| `skipped`         | 400                            | → `done`; no Google                   | 200 no-op                                | → `pending`; **clear** ids                               | 400 — reopen is the undo                                     |
...
- Pause while running must close the chip and **clear** the stored ids (a later start mints a new event). Pause is the dedicated abort; reopen of `in_progress` stays 400.
...
| Surface           | This train                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Home (`/`)**    | The working surface. Date selector (prev / today / next + calendar pick). Mixed Agenda list for the selected date — a ranked **living** pile plus a **Completed** dump below it (newest embed `updated_at` first; see the 2026-08-24 amendment). Add-task picker. Reorder. Check-off. Skip. Start. Pause (in-progress tasks and occurrences). Tap occurrence to rename. Tap task → existing `TaskModal`. Replace the mock `todayItems` list; **remove** the mock timeline (`SkewedTimeline`) from Home so the page is the Agenda, not two competing UIs. |
...
Shapes are locked here; the Rust module layout is not. Endpoints are session-gated like lists/categories. Google token refresh happens only when a handler would write Google: occurrence `/start`, occurrence `/pause` / `/complete` / `/skip` when `in_progress` with stored ids, and occurrence title `PATCH` when a chip exists.
...
| `POST /api/occurrences/:id/pause`       | `in_progress` → `pending`; closes an open event if one is running, then **clears** chip ids. `pending` → 200 no-op. `done`/`skipped` → 400.                                                                                                                                                                                                                                                                                                                                                                          |
