...
import type { Task, TimeBlock as TimeBlockType, TaskEvent, TimelineItem } from '../types'
...
function useSkewedTimeline(items: TimelineItem[]): UseSkewedTimelineReturn {
  ...
  const { uniqueHours, itemPositions, totalHeight } = useMemo(() => {
    ...
    if (!measured || naturalHeights.size === 0) {
      return { uniqueHours: hours, itemPositions: positions, totalHeight: 24 * MIN_HOUR_HEIGHT }
    }
    ...
    for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
      ...
      const rowHeight = Math.max(
        ...row.map(item => naturalHeights.get(item.id) || (isTimeBlock(item) ? 100 : 40))
      )
      ...
      row.forEach((item, index) => {
        const naturalHeight = naturalHeights.get(item.id) || (isTimeBlock(item) ? 100 : 40)
    ...
    // Deduplicate hours
    const uniqueHours = hours.filter((h, index, self) =>
      index === self.findIndex((t) => t.hour === h.hour)
    )

    const totalHeight = uniqueHours.length > 0
...
export function SkewedTimeline({ items, className, onItemsChange }: SkewedTimelineProps) {
  const { measured, measureRef, uniqueHours, itemPositions, totalHeight } = useSkewedTimeline(items)
  ...
  const handleSaveItem = (updates: { title: string; startTime: string; endTime: string }) => {
  ...
  return (
    <div className={cn('relative', className)}>
      ...
      {!measured && (
        <div ref={measureRef} className="absolute opacity-0 pointer-events-none left-0 top-0 w-64">
