...
import { todayBlocks, streams } from '@/app/mock-data'
import { cn } from '@/lib/utils'

const WEEK_DAYS = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']

interface CalendarEventProps {
  block: typeof todayBlocks[0]
  compact?: boolean
}
...
function CalendarEvent({ block, compact }: CalendarEventProps) {
  return (
    ...
    >
      <p className={cn('font-medium', compact && 'truncate')}>{block.streamName}</p>
      {!compact && (
                    ...
                    <>
                      <span
                        className={cn(
                          ...
                          isToday
                            ? 'bg-sanctuary-green text-white font-semibold'
                            : 'text-foreground'
                        )}
                      >
                        {day}
                      ...
                      {day === 6 && (
                        <div className="mt-2 space-y-1">
                          {todayBlocks.slice(0, 2).map((block) => (
                            <CalendarEvent key={block.id} block={block} compact />
                          ))}
                        </div>
                      )}
                      ...
                      {[3, 8, 10, 15, 20, 22, 25].includes(day) && (
                        <div className="mt-2">
                          <div
                            ...
                            style={{ 
                              backgroundColor: streams[Math.floor(Math.random() * streams.length)].color 
                            }}
                          />
                        </div>
                      )}
              ...
              {todayBlocks.map((block) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-foreground">{block.streamName}</p>
